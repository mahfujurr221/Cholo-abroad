...
<x-modern.filter title="Filter Testimonial" icon="bx bx-search-alt" :resetUrl="route('testimonials.index')" :expanded="request()->anyFilled(['search'])">
    <div class="col-md-12">
        <x-modern.input label="Search name" name="search" placeholder="Search..." :value="request('search')" icon="bx bx-search" />
    </div>
</x-modern.filter>

<x-modern.card title="Testimonial List" icon="bx bx-list-ul">
    <x-slot name="actions">
        @can('create-testimonial')
        <x-modern.actions.button tag="a" href="{{ route('testimonials.create') }}" actionType="add" label="Add New" size="sm" />
        @endcan
    </x-slot>
    ...
    <x-modern.table :headers="['#', 'name', 'Status', 'Actions']">
        ...
        <tr>
            <td class="align-middle">{{ $loop->iteration + ($testimonials->currentPage() - 1) * $testimonials->perPage() }}</td>
            ...
            <td class="align-middle">
                <div class="d-flex gap-2">
                    ...
                    <x-modern.actions.button tag="a" href="{{ route('testimonials.edit', $item->id) }}" actionType="edit" outline />
                    @endcan
                    @can('delete-testimonial')
                    <form action="{{ route('testimonials.destroy', $item->id) }}" method="POST" class="d-inline-block">
                        @csrf
                        @method('DELETE')
